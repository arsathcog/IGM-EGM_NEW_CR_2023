create or replace PACKAGE BODY RCL_IGM_VESEEL_VOYOAGE_INFO AS
 ...
 PROCEDURE RCL_IGM_SAVE_VESEEL_VOYOAGE_SAVE_DATA(   
     ...
     V_SQL:='DELETE FROM IGM_VESSEL_VOYAGE_DETAILES   WHERE SERVICE in('||''''|| P_I_V_SERVICE||''''||') and POD in('||''''|| P_I_V_POD||''''||') and POD_TERMINAL in('||''''|| P_I_V_POD_TERMINAL||''''||') and VESSEL in('||''''|| P_I_V_VESSEL||''''||') and VOYAGE in('||''''|| P_I_V_VOYAGE||''''||')';   
           ...
           (
            (SELECT DATA_SET FROM IGM_DATA_SET_JSON), '$[*]'
                COLUMNS (    
							...
							ROAD_CARR_CODE PATH '$.roadCarrCodeVVS',
							ROAD_TP_BOND_NO PATH '$.tpbondnoVVS',
                            ...
                            TERMINAL_PORT PATH '$.podTerminalPort',
 ...
 PROCEDURE RCL_IGM_GET_SAVED_VESEEL_VOYOGE_DATA(
  ...
  --insert into chandra values ('xxxxxx');  commit;
 ...
 OPEN P_O_REFIGMTABFIND FOR 
                SELECT
                        ...
                        ''  BL_NO,
                       (select VESSEL_NAME 
                       from RCLTBLS.CAM_VESSEL 
                       where 
                       PK_VESSEL=P_I_V_VESSEL)      VESSEL_NAME,
                        CUST_CODE                   CUSTOM_CODE,
                     	CUST_CODE                   , 
                        CALL_SIGN                   , 
                        LINE_CODE                   , 
                        AGENT_CODE                  , 
                        ...
                        NO_OF_CREW                    , 
                        REMARK_VESSEL                 ,
                        light_due          as   LIGHT_DUE                ,
                        TERMINAL_PORT      as   POD_TERMINAL_PORT,  
                        ''                 as   POL_TERMINAL_PORT ,
                        SENDER_ID,
                        RECIEVER_ID,   
                        AUTHREPRSNTVCD,
                        LAST_PORT_1_NAME,
                        ...
                        VESSEL = P_I_V_VESSEL  AND  VOYAGE =  P_I_V_VOYAGE    AND POD = P_I_V_POD)                              noOfCrew 
                FROM
...
PROCEDURE RCL_IGM_SAVE_PERSON_DETAILES(   
      ...
      V_SQL varchar(1000);
        BEGIN  
        --delete from chandra ; commit;
--insert into chandra values ('P_I_V_BL  ==>   '||P_I_V_BL); commit;
--insert into chandra values ('P_I_C_CONT_LST'||P_I_C_CONT_LST); commit;
          ...
          SELECT --SYBLNO                            BL_NO, 
               ...
               --FROM_TERMINAL                       POL_TERMINAL,
                 ...
                 (SELECT PARTNER_VALUE
                  ...
                  ) AND SEALINER_VALUE=TEMP_POD_TERMINAL AND ROWNUM=1)                         POD_TERMINAL_PORT, 
                  ''                                                        POL_TERMINAL_PORT,
                ...
                (     SELECT PARTNER_VALUE
                  ...
                  ) AND SEALINER_VALUE=P_I_V_POD AND ROWNUM=1) CUSTOM_CODE,
...
--                 EYBLNO=SYBLNO )                                          NET_PACKAGE,
            ...
            , (SELECT A.VSCLSN 
              ...
              ACT_VOYAGE_NUMBER  PARENT_VOY,--keep it blank (its in icl vsl page) by guru
...
PROCEDURE RCL_IGM_CREW_EFFECTS_FORMAT(   
     ...
     V_SQL:='DELETE FROM IGM_CREW_EFFECT_FORMATE   WHERE vessel = ('''|| P_I_V_VESSEL ||''') 
              AND  VOYAGE  = ('''|| P_I_V_VOYAGE ||''')  
              AND  POD =  ('''|| P_I_V_POD ||''')'; 
      EXECUTE IMMEDIATE  V_SQL; 
     COMMIT; 
	...
	PROCEDURE RCL_IGM_SHIP_STORE_FORMAT(   
     ...
     V_SQL:='DELETE FROM IGM_SHIP_STORE_FORMAT    WHERE VESSEL = ('''|| P_I_V_VESSEL ||''') 
              AND  VOYAGE  = ('''|| P_I_V_VOYAGE ||''')  
              AND  POD =  ('''|| P_I_V_POD ||''')   '; 
      EXECUTE IMMEDIATE  V_SQL; 
     COMMIT; 
...
POD,
VESSEL_SRNO
FROM IGM_SHIP_STORE_FORMAT WHERE  
  VESSEL=('||''''|| P_I_V_VESSEL||''''||')AND VOYAGE=('||''''|| P_I_V_VOYAGE||''''||')';
...
END RCL_IGM_VESEEL_VOYOAGE_INFO;