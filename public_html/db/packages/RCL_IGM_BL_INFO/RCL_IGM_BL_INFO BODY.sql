create or replace PACKAGE BODY RCL_IGM_BL_INFO 
IS 
    ...
    PROCEDURE RCL_IGM_GET_MASTER_CONSIGNEE(  
        ...
        BEGIN   
            ...
            l_v_sql:=   'SELECT 
                ...
                (CASE WHEN
                   INSTR( UPPER(    SUBSTR((SELECT CYNAME    FROM sealiner.IDP030   WHERE CYBLNO IN (FK_BL_NO)   AND CYRCTP   = ''C'' --1ST
                      AND rownum   =1
                      ),1,70) ),''BANK'')>0 OR 
        ...
        (CASE WHEN
                ...
                FROM RCLTBLS.DEX_BL_CUSTOMERS WHERE  CUSTOMER_TYPE =''C'' and FK_BL_NO IN ('||P_I_V_BL||')';
       ...
       BEGIN   
        ...
        (CASE WHEN
                ...
                ELSE 
                 ...
                 from IGM_CONSIGNEE_TABLE_TWO WHERE PK_BL_NO IN ('||P_I_V_BL||')';
           ...
           BL_NO_INPUT VARCHAR2(32000); 
            ...
            BEGIN   
                ...
                l_v_sql:=   ' SELECT  PK_BL_NO FK_BL_NO,
                            ...
                            FROM  IGM_CONTAINER_DETAILS_TWO WHERE 
                            PK_BL_NO IN ('||P_I_V_BL||')';
        ...
        P_O_V_ERROR        OUT VARCHAR2) 
           ...
           V_SQL varchar2(32000);
            ...
            BEGIN   
                    ...
                    SELECT
                        to_terminal
                    INTO temp_pod_terminal
                    FROM
                        idp005
                    WHERE
                        act_vessel_code = p_i_v_vessel
                        AND act_voyage_number = p_i_v_voyage
                        AND discharge_port = p_i_v_pod
                    ...
                    OPEN p_o_refigmtabfind FOR SELECT
                           ...
                           (
                               SELECT
                                   partner_value
                               FROM
                                   edi_translation_detail
                               WHERE
                                   eth_uid IN (
                                       SELECT
                                           eth_uid
                                       FROM
                                           edi_translation_header
                                       WHERE
                                           code_set = 'IGMTML'
                                   )
                                   AND sealiner_value = temp_pod_terminal
                                   AND ROWNUM = 1
                           ) pod_terminal,
                           (
                               SELECT
...
--janmejaya(26-05-2021) mapping given by (by guru)
                           ...
                           'INR' currency,--INR
                           'P' modeof_tp_fee,--keep it blank (its in icl bl cargo page)
                           ...
                           (
                               ...
                               WHERE
                                   pk_bl_no = syblno --) -- POR   
                                   AND ROWNUM < 2
                           ) port_of_destination,--Place of delivery (respective code from master)
                           (
                               SELECT
                                   dn_pol
                               FROM
                         ...
                         (SELECT port_name FROM rcltbls.cam_port
                           ...
                           ( SELECT
                               ...
                               WHERE
                                   a.eth_uid IN (
                                       ...
                                       WHERE
                                           eth.standard = 'EDIFACT'
                                           AND eth.code_set = 'IGMPORT'
                                   )
                                   AND a.sealiner_value = (
                                       SELECT
                                           dn_pld
                                       FROM
                                           rcltbls.dex_bl_header
                                       WHERE
                                           pk_bl_no = syblno
                                   ) -- Port of Descharged/CFS 
                                   AND ROWNUM < 2
                           ) port_of_descharged_cfs,--We Will update 
                           ...
                           (
                               ...
                               WHERE
                                   pk_bl_no = syblno
                                   AND ROWNUM = 1
                           ) gros_weight,--From DCS grs.wt.column 				                
                           '' unit,--From DCS unit   				                
                           (select sum(METRIC_MSMT) 
                           from rcltbls.dex_bl_commodity 
                           where FK_BL_NO=syblno) volume,--From DCS volume	 	               
                           (
                               CASE
                                   WHEN aympod = aydest THEN
                                       'LC'
                                   ELSE
                                       'TI'
                               END
                           ...
                           (
                               ...
                               FROM
                                   ...
                                   sealiner.edi_translation_detail   dtl
                               WHERE
                                   hdr.code_set = 'IGMPORT'
                                   AND hdr.eth_uid = dtl.eth_uid
                                   AND sealiner_value = p_i_v_pod
                                   AND ROWNUM = 1
...
--  ''                                                              EQUIPMENT_SEAL_TYPE,
                           ...
                           (
                               ...
                               FROM
                                   itp130
                               WHERE
                                   tqterm = to_terminal-- DEL FROM BL 
                                   AND tqrcst = 'A'
                                   AND ROWNUM = 1
                        ...
                        (select NET_MSMT_METRIC from  RCLTBLS.dex_bl_containers
                                     where FK_BL_NO= SYBLNO and  ROWNUM = 1)                 gross_volume,
                           'UOV' unit_of_volume,
...
--VVPCSQ =PORTSEQUENCE' AND
                           ...
                           (
                               SELECT
                                   fk_unno
                               FROM
                                   rcltbls.dex_bl_special_cargo_dtl   sp,
                                   rcltbls.dex_bl_commodity           cm
                               WHERE
                                   cm.dn_special_hndl = 'D1'
...
--END
                           ...
                           (
                               ...
                               WHERE
                                   ...
                                   AND fk_house_bl_no IS NOT NULL
                                   AND ROWNUM = 1
                           ) hbl_no,
                           '' from_item_no,
                           '' to_item_no,
                           ...
                           'FCL' ne_cargo_movmnt,
                           '' consignee_name,
                           (select FLAG_DG from RCLTBLS.dex_bl_commodity WHERE FK_BL_NO = syblno) flag_dg,
                            (select DN_COMMODITY_CODE from RCLTBLS.dex_bl_commodity WHERE FK_BL_NO = syblno) COMMODITY_CODE,
                            (select DN_PACKAGE_KIND from RCLTBLS.dex_bl_commodity WHERE FK_BL_NO = syblno) PACKAGE_KIND,
                            (select COMMODITY_SEQ from RCLTBLS.dex_bl_commodity WHERE FK_BL_NO = syblno) COMMODITY_SEQ ,
                            
                            (select METRIC_MSMT from  RCLTBLS.dex_bl_containers
                             WHERE FK_BL_NO = syblno AND rownum =1)  cargo_msmt,
                             (CASE WHEN IDP5.DISCHARGE_PORT = (select DISCHARGE_PORT from IDP005 where DISCHARGE_PORT IN (p_i_v_pod) AND ROWNUM=1)  
                             THEN 'DISCHARGED' 
                                  ELSE 'NOT'
                                     END)  FLAG_DISCHARGE,
                        ...
                        (select  DN_PLR   from RCLTBLS.DEX_BL_HEADER where PK_BL_NO = SYBLNO ))  RECIEPT_NAME,
                        (select  DN_PLR   from RCLTBLS.DEX_BL_HEADER where PK_BL_NO = SYBLNO)    DN_PLR,
                               
                         (SELECT GST_STATE_CODE FROM SEALINER.ITP185 WHERE STCODE IN (
                          SELECT STATE FROM RCLTBLS.DEX_BL_CUSTOMERS WHERE FK_BL_NO =SYBLNO AND ROWNUM = 1)and rownum = 1)   GST_STATE_CODE,
                           
                            (SELECT POINT_NAME  from rcltbls.cam_point WHERE PK_POINT_CODE IN 
                            (select  DN_PLD   from RCLTBLS.DEX_BL_HEADER where PK_BL_NO = SYBLNO))  ACCEPTANCE_NAME,
                           (select  DN_PLD   from RCLTBLS.DEX_BL_HEADER where PK_BL_NO = SYBLNO )    DN_PLD,
                            ...
                            (SELECT PK_DISCHARGE_LIST_ID  FROM   VASAPPS.TOS_DL_discharge_LIST WHERE 
                                ...
                                FK_BL_NO =  SYBLNO) and dn_equipment_no  IN (select  DN_CONTAINER_NO  FROM RCLTBLS.DEX_BL_CONTAINERS  
        ...
        P_O_V_ERROR        OUT VARCHAR2) 
            ...
            BEGIN   
                ...
                FOR i IN (
                    ...
                    INSERT INTO dex_bl_header_temp 
                                    (mbl_no, ma_seq_no,  dn_pod,  dn_pol, met_mt, met_wt, dn_plr, dn_pld,point_name,reciept_point_name,hbl_count) 
                                    SELECT 
                                        pk_bl_no,
                                        ma_seq_no,
                                        dn_pod,
                                        ...
                                        (SELECT point_name FROM rcltbls.cam_point  WHERE
                                         ...
                                         (SELECT point_name FROM rcltbls.cam_point  WHERE
                                         pk_point_code = dn_plr  AND ROWNUM = 1) reciept_point_name,
                                    fk_house_bl_no
                                    FROM
                                        rcltbls.dex_bl_header
                                    WHERE
                                        pk_bl_no = SUBSTR(i.l,2,LENGTH(i.l)-2) and rownum = 1 ;
                     commit;
                ...
                V_SQL:=   'select 
                                ...
                                met_wt                                                                                          GROS_WEIGHT,
                                dn_plr                                                                                          dn_plr,
                                reciept_point_name                                                                                         RECIEPT_NAME,
                                point_name                                                                                          ACCEPTANCE_NAME,
                                dn_pld                                                                                          dn_pld,
                                ...
                                ''FCL''                                                                                         NE_CARGO_MOVMNT,
                                FLAG_DG                                                                                         FLAG_DG,
                                DN_COdbct.MMODITY_CODE                                                                               COMMODITY_CODE,
                                DN_PACKAGE_KIND                                                                                 PACKAGE_KIND,
                                COMMODITY_SEQ                                                                                   COMMODITY_SEQ,
     ...
     PROCEDURE  RCL_IGM_SAVE_UNFETCHED_BL_DATA_SUSHIL ( 
            ...
            BEGIN  
                ...
                FOR i IN (
                    ...
                    delete from dex_bl_header_temp where mbl_no = SUBSTR(i.l,2,LENGTH(i.l)-2);commit;
                    INSERT INTO dex_bl_header_temp (mbl_no, ma_seq_no,  dn_pod,  dn_pol, met_mt, met_wt, dn_plr, dn_pld,  point_name,reciept_point_name,  hbl_count) 
                                    SELECT 
                                        pk_bl_no,
                                        ma_seq_no,
                                        dn_pod,
               ...
               WHERE ( P_I_V_SERVICE IS NULL
                ...
                IF BL_COUNT = 0 THEN   
                    ...
                    WHERE   
                     IDP5.ACT_VESSEL_CODE = P_I_V_VESSEL
                     AND IDP5.ACT_VOYAGE_NUMBER = P_I_V_VOYAGE
                     AND (select TO_DATE(POL_ARRIVAL_DATE,'yyyy-mm-dd') from IDP005 where DISCHARGE_PORT = P_I_V_POD
