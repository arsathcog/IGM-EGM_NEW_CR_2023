...
public class IGMNewScreenSvc extends BaseAction {
	...
	//------------------------------------------reading and returnig the ACK json file-------------------------------------- 
...
//	ImportGeneralManifestInput     saveParam 			= mapper.readValue(data, ImportGeneralManifestInput.class);
	...
	//Update
			private ActionForward applyIgm(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response) throws Exception {
...
//		jsonObj.put("result", getUniqueRecords(resultBL1));
	...
	private ActionForward getSelectAllOption(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (objForm.getSavedBlList() != null && !objForm.getSavedBlList().equals("")) {
			...
			objPreviousDao.setPreviousDeclData(blObjTmp, IGMPPreviousDeclarationDao.RCL_IGM_GET_SAVE_PREV_DECLARATION);
	...
	private ActionForward getVesselVoyagData(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		Map<Object, Object> mapReturn = objDao.getVesselVoyagData(mapParam ,IGMDaoNew.SQL_GET_SAVED_IGM_VESSLE_VOYAGE_DATA);
		...
		/* BL Start.. */
		 Map<Object, Object> mapReturnBL = objDao.getBLDataNew(mapParam, IGMDaoNew.SQL_GET_IGM_BL_MASTER_SAVE_DATA, false);
	...
	private List<ImportGeneralManifestMod> getBlDetails(ImportGeneralManifestMod mod ,ImportGeneralManifestUim objForm) throws Exception {
		...
		if (objForm.getSavedBlList() != null && !objForm.getSavedBlList().equals("")) {
			...
			objConsigneeDao.setConsigneeData(blObjTmp, IGMConsigneeDataDao.RCL_IGM_GET_SAVE_CONSIGNEE);
			objNotifyPartyDao.setNotifyPartyData(blObjTmp, IGMNodifyPartyDao.RCL_IGM_GET_SAVE_NODIFY_PARTY_DESCRIPTION_IMPORT);
			objMarksDescDao.setMarksDescriptionData(blObjTmp, IGMMarksAndDescDao.RCL_IGM_GET_SAVE_MARKS_DESCRIPTION);
			objPreviousDao.setPreviousDeclData(blObjTmp, IGMPPreviousDeclarationDao.RCL_IGM_GET_SAVE_PREV_DECLARATION);
			blObj.addAll(blObjTmp);
			}
		}else {
		 ...
		 * IGMDaoNew.SQL_GET_IGM_BL_MSTR_DATA_NEW, true,true);
